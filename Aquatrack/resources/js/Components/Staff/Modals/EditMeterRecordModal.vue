                        ...
                        <div class="flex-1 overflow-y-auto p-6">
                            ...
                            >
                                ...
                                <div
                                    ...
                                    "
                                    class="bg-red-50 dark:bg-red-900/20 p-4 rounded-lg border border-red-200 dark:border-red-700 mb-4"
                                >
                                    <div class="flex items-start gap-3">
                                        <div
                                            class="p-2 bg-red-100 dark:bg-red-800 rounded mt-0.5"
                                        >
                                            <v-icon
                                        ...
                                        <div>
                                            ...
                                            >
                                                Water Rates Not Configured
                                            </div>
                                            <p
                                                class="text-sm text-red-700 dark:text-red-300 mt-1"
                                            >
                                                Cannot calculate amount. Please
...
const calculateBillAmount = (consumption) => {
    ...
    // If no water rates are available, return 0
...
// Watch reading changes to auto-calculate amount using dynamic rates
watch(
    () => form.reading,
    (newReading) => {
        if (newReading !== "" && !isNaN(newReading)) {
            // Auto-calculate the amount based on consumption using dynamic water rates
...
const calculatedAmount = computed(() => {
    ...
    return calculateBillAmount(consumption);
});
...
const submitForm = async () => {
    ...
    // Check if water rates are available
    if (!waterRates.value || waterRates.value.length === 0) {
        Swal.fire({
            icon: "error",
            title: "Water Rates Not Configured",
            text: "Cannot update reading. Water rates are not configured in the system.",
            customClass: {
                popup: "!z-[1300]",
                container: "!z-[1300]",
            },
    ...
    try {
        const result = await Swal.fire({
            ...
            html: `Update meter reading for <strong>${
                props.user.name
            }</strong>?<br><br>
                   <div class="text-left">
                     <strong>Billing Month:</strong> ${
                         props.record.billing_month
                     } ${props.record.year}<br>
                     <strong>Reading:</strong> ${form.reading} m³<br>
                     <strong>Consumption:</strong> ${
                         calculatedConsumption.value
                     } m³<br>
                     <strong>Amount:</strong> ₱${calculatedAmount.value.toFixed(
                         2
                     )}<br>
        ...
        if (result.isConfirmed) {
            const response = await axios.put(
                route("staff.reading.update", { readingId: props.record.id }),
                {
                    reading: parseFloat(form.reading),
                    amount: calculatedAmount.value, // Use calculated amount from dynamic rates
                    status: form.status,
        ...
        Swal.fire({
            ...
            text:
                ...
                error.message ||
                "There was an error updating the meter reading",
            customClass: {
                popup: "!z-[1300]",
                container: "!z-[1300]",
            },
